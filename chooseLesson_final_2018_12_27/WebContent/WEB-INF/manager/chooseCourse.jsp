...
<body>
	...
	<form action="Manager?username=${username}" method="post">
		<input type="submit" name="manager" value="viewCourse" />
	</form>
	<div align="center">
		<h1>选课表</h1>
	</div>
		...
		<table border="1">
			...
			<c:forEach items="${list }" var="vars">
				<tr>
					...
					<td>${vars.limited}</td>
					<td><form
							action="ChooseCourseController?courseid=${vars.course_id }&username=${username}"
							method="post">
							<input type="submit" name="function" value=chooseCourse />
						</form></td>
				</tr>
			</c:forEach>
		</table>
